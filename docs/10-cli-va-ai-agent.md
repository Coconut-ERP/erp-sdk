...
- Bốn skill `erp-miniapp`, `erp-data`, `erp-tools` và `erp-wiki` — gói hướng dẫn cài vào Claude Code
...
```bash
erp skill install                        # → ~/.agents/skills/{erp-miniapp,erp-data,erp-tools,erp-wiki}
...
erp-miniapp (dựng app), erp-data (khai thác dữ liệu), erp-tools (workflow, tệp
và các công cụ khác của ERP), erp-wiki (viết và truy hồi wiki của workspace).
```
...
| **`erp-data`** | Khai thác workspace có sẵn: đọc schema thật, query có filter/sort/phân trang, tránh N+1 qua `relation`, tổng hợp bằng `DataFrame` hoặc SQL read-only, ghi và ghi hàng loạt an toàn sau dry run, và dashboard. References: `api.md`, `recipes.md`, `sql.md` |
| **`erp-tools`** | Dùng các công cụ của ERP ngoài record: workflow (script hoặc agent — trigger, publish, env, run, sandbox của runner và vòng `check` → `testRun`), drive (upload, tải về, chia sẻ, thùng rác), shared variable, hội thoại copilot và task board AI cá nhân. References: `workflows.md`, `workflow-runtime.md`, `workflow-patterns.md`, `workflow-testing.md`, `agent-workflows.md`, `files.md`, `task-board.md` |
